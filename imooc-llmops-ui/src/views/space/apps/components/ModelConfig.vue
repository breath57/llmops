...
const changeModel = (value: any): any => {
  // 2.1 使用/拆分出提供商+模型名字
  // 只以第一次出现 / 的位置进行分割, 兼容名称为 deepseek-ai/DeepSeek-V3 的模型
  const index = value.indexOf('/')
  const provider_name = value.substring(0, index)
  let model_name = value.substring(index + 1)
...
const hideModelTrigger = () => {
  // 3.1 处理表单数据
  // 只以第一次出现 / 的位置进行分割, 兼容名称为 deepseek-ai/DeepSeek-V3 的模型
  const index = form.value.selectValue.indexOf('/')
  const provider_name = form.value.selectValue.substring(0, index)
  const model_name = form.value.selectValue.substring(index + 1)
